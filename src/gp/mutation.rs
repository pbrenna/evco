...
impl Mutation {
    ...
    pub fn uniform() -> Mutation {
        Mutation {
            mode: MutationMode::Uniform,
        }
    }
    /// Perform mutation by randomly replacing a node with a new subtree and pruning.
    pub fn uniform_prune(d: usize) -> Mutation {
        Mutation {
            mode: MutationMode::UniformPrune(d),
        }
    }
    ...
    pub fn node_replacement() -> Mutation {
        Mutation {
            mode: MutationMode::NodeReplacement,
        }
    }
    ...
    pub fn ephemeral_one() -> Mutation {
        Mutation {
            mode: MutationMode::Ephemeral(EphemeralMode::One),
        }
    }
    ...
    pub fn ephemeral_all() -> Mutation {
        Mutation {
            mode: MutationMode::Ephemeral(EphemeralMode::All),
        }
    }
        ...
        R: Rng,
    {
        match self.mode {
            ...
            MutationMode::UniformPrune(depth) => self.mutate_uniform_prune(indv, tg, config, depth),
        }
    }
    ...
    fn mutate_uniform_prune<T, R>(
        ...
        R: Rng,
    {
        self.mutate_uniform(indv, tg, config);
        indv.prune_at(max_depth);
    }
